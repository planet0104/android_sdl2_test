...
fn main(){
    ...
    println!("CPU {:?} 耗时:{}ms", result.get(0..15), duration_to_milis(&start_time.elapsed()));
    
    //gpu判断是否质数
    let start_time = Instant::now();
    check_prime_gpu(&pro_que, &mut numbers, &mut result).unwrap();
    println!("GPU {:?} 耗时:{}ms", result.get(0..15), duration_to_milis(&start_time.elapsed()));
}
